  ...
  private void shortestHops() {
    if (G == null) {
      ...
      if (!G.marked[destination]) {
        ...
        // Construct the shortest path using a stack
        Stack<Integer> path = new Stack<>();
        int x;
        for (x = destination; x != source; x = G.edgeTo[x]) {
          path.push(x);
        }
        path.push(source);

        // Print the shortest distance and the path with the weight of each edge
        int hops = G.distTo[destination];
        System.out.println(
            "Shortest route from " + cityNames[source] + " to " + cityNames[destination] + " (" + hops + " hops):");
        int current = path.pop();
        while (!path.isEmpty()) {
          int next = path.pop();
          for (WeightedDirectedEdge e : G.adj(current)) {
            if (e.to() == next) {
              System.out.println(cityNames[current] + " to " + cityNames[next] + " (" + e.weight() + " miles)");
              break;
            }
          }
          current = next;
  ...
  private class Digraph {
    ...
    public void dijkstras(int source, int destination) {
      ...
      while (nMarked < this.v) {
        ...
        // Update marked[] and nMarked
        if (current == -1) {
          // Disconnected graph, there is no path from source to destination
          break;
        } else {
          marked[current] = true;
          nMarked++;
