  ...
  private void shortestHops() {
    if (G == null) {
      ...
      if (!G.marked[destination]) {
        ...
        // Print the number of hops (from the distTo array) and the path
        int hops = G.distTo[destination];
        System.out.print("The shortest route from " + cityNames[source] + " to " + cityNames[destination] + " has " + hops
            + " hop(s):");
        while (!path.isEmpty()) {
