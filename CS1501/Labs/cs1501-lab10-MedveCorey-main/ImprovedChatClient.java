...
public class ImprovedChatClient extends JFrame implements Runnable, ActionListener {
    ...
    public ImprovedChatClient ()
    {
        ...
        myWriter.println(HANDSHAKE_MSG);
    ...
    {
        try {
            // Wait for server's response to handshaking message
            String response = myReader.readLine();
            if (response.equals("Hello")) {
                outputArea.append("Handshaking successful.\n");
            } else {
                outputArea.append("Handshaking failed.\n");
                return;
            }
        } catch (IOException e) {
